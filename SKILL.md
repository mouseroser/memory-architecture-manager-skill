...
根据 agent 人格类型，自动配置记忆系统：

| Agent 类型 | 记忆系统 | memory-lancedb-pro | 说明 |
|-----------|---------|-------------------|------|
| **功能型** (functional) | 无 | ❌ 不启用 | 一次性任务，无需记忆 |
| **职能型** (operational) | 文件系统 | ❌ 禁用 | 通过 memory/*.md 维护记忆 |
| **独立人格型** (independent) | 完整 | ✅ 启用 | 独立 LanceDB + 文件系统 |

### 配置逻辑

```bash
# optimize-memory-architecture.sh 会自动：

# 1. 检测 agent 类型（调用 lib-agent-types.sh）
# 2. 根据类型配置：
...
# 职能型：
#   - 创建 memory/ 目录
#   - 在 agentDir/openclaw.json 中禁用 memory-lancedb-pro

# 独立人格型：
#   - 创建完整 memory/ 结构（topics/ + runtime-maintenance/）
#   - 在 agentDir/openclaw.json 中启用 memory-lancedb-pro
#   - 配置独立的 LanceDB 路径
#   - 创建 MEMORY.md 索引文件
```
| **极简配置** | ❌ | ❌ | 只用内置功能 | ⭐⭐ |
